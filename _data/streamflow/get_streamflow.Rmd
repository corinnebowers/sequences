...
Peak flow statistics come from USGS StreamStats, which are calculated from the method cited below. 
...
## download peak flow statistics from batch processor

```{r}
## figure out what row names mean
labeldf <- 
  st_read(file[1], layer = 'FLOWSTATS', quiet = TRUE) %>% 
  group_by(StatName) %>% 
  summarize(StatLabel = StatLabel[1]) %>% 
  separate(StatName, sep = '-', into = c('AEP', NA), remove = FALSE) %>% 
  mutate(AEP = toNumber(AEP)/100, RP = 1/AEP) %>% 
  arrange(RP) %>% 
  mutate(newlabel = paste0('RP', RP))

```

```{r}
## reload file names for batch processor results
file_id <- 
  c('1133046413374416881', 
    '2133046413629627867', 
    '3133046413829478084', 
...
## pull peak flow statistics (cfs) for available gages
flowstats1 <- 
  map_dfr(
    .x = file,
    .f = ~st_read(.x, layer = 'FLOWSTATS', quiet = TRUE) %>%
      mutate(Value = toNumber(Value)) %>%
      group_by(Name) %>%
      filter(!('areaave' %in% RegionID) | RegionID == 'areaave') %>%
      filter(AreaPercent == max(AreaPercent)) %>%
      pivot_wider(
        id_cols = 'Name',
        names_from = 'StatLabel',
        values_from = 'Value') %>%
      column_to_rownames('Name') %>%
      mutate(sumcol = apply(., 1, sum)) %>%
      filter(sumcol > 0) %>%
      select(-sumcol) %>%
...
## download peak flow statistics manually for remaining gages

```{r}
## identify which gages need to be run manually
gages$site_no[which(!(gages$site_no %in% flowstats1$site_no))] %>% data.frame(x = .)

```

For the list of gages above, we ran each one through the interactive StreamStats online application manually and saved the results as individual files.

note: I've done 240/275 so far (9/30/22)

11335000 11336585 11364300 11372350 11374000

```{r}
## check drainage geometries for manually downloaded gages
sites <- list.files('_data/streamflow/flowstats')
drainage_check <- 
 map_dfr(
   .x = sites,
   .f = ~paste0('_data/streamflow/flowstats/', .x, '/layers/globalwatershed.shp') %>% 
     st_read(quiet = TRUE) %>% 
     mutate(site_no = .x)) %>% 
 st_make_valid %>%
 st_transform(st_crs(drainage)) %>% 
 mutate(area_check = toNumber(st_area(.))*meter2feet^2) %>% 
 select(site_no, area_check) %>% 
 st_drop_geometry %>% 
 left_join(drainage %>% st_drop_geometry, by = 'site_no') %>% 
 mutate(error = (area-area_check)/area)
drainage_check %>% arrange(desc(error))

## keep only gages where the drainage geometries match results from api & batch
# drainage_check %>% arrange(desc(error))
sites <- drainage_check %>% filter(error <= 0.01) %>% pull(site_no)

```

```{r}
## load flowstats for manually downloaded gages
flowstats2 <- 
  map_dfr(
    .x = sites,
    .f = ~paste0('_data/streamflow/flowstats/', .x, '/FLOWSTATS.dbf') %>% 
      st_read(quiet = TRUE) %>% 
      mutate(Value = toNumber(Value)) %>%
      filter(grepl('AEP', Code)) %>% 
      filter(!('Area-Averaged' %in% Region) | Region == 'Area-Averaged') %>%
      pivot_wider(
        id_cols = 'Name',
        names_from = 'Code',
        values_from = 'Value') %>% 
...
```{r}
## combine peak flow stats from batch processor & manual downloads
all(names(flowstats1) == names(flowstats2))
flowstats <- rbind(flowstats1, flowstats2)

```

## plot normalized peak ratios

```{r fig.width = 7, fig.height = 4}
## normalize w.r.t. 2-year peak
flowstats_norm <- flowstats %>% 
  column_to_rownames('site_no') %>% 
  apply(1, function(x) x/x[1]) %>% 
  t %>% as.data.frame %>% 
  rownames_to_column('site_no') 

g005 <- ggplot(flowstats_norm) + 
  geom_histogram(
    aes(x = PK20AEP), 
    color = 'black', fill = 'grey90', bins = sqrt(nrow(flowstats_norm))) + 
  scale_x_continuous('5-Year / 2-Year Flow Ratio') + 
  scale_y_origin()
g010 <- ggplot(flowstats_norm) + 
  geom_histogram(
    aes(x = PK10AEP), 
    color = 'black', fill = 'grey90', bins = sqrt(nrow(flowstats_norm))) + 
  scale_x_continuous('10-Year / 2-Year Flow Ratio') + 
  scale_y_origin()
g025 <- ggplot(flowstats_norm) + 
  geom_histogram(
    aes(x = PK4AEP), 
    color = 'black', fill = 'grey90', bins = sqrt(nrow(flowstats_norm))) + 
  scale_x_continuous('25-Year / 2-Year Flow Ratio') + 
  scale_y_origin()
g050 <- ggplot(flowstats_norm) + 
  geom_histogram(
    aes(x = PK2AEP), 
    color = 'black', fill = 'grey90', bins = sqrt(nrow(flowstats_norm))) + 
  scale_x_continuous('50-Year / 2-Year Flow Ratio') + 
  scale_y_origin()
g100 <- ggplot(flowstats_norm) + 
  geom_histogram(
    aes(x = PK1AEP), 
    color = 'black', fill = 'grey90', bins = sqrt(nrow(flowstats_norm))) + 
  scale_x_continuous('100-Year / 2-Year Flow Ratio') + 
  scale_y_origin()
g200 <- ggplot(flowstats_norm) + 
  geom_histogram(
    aes(x = PK0_5AEP), 
    color = 'black', fill = 'grey90', bins = sqrt(nrow(flowstats_norm))) + 
  scale_x_continuous('200-Year / 2-Year Flow Ratio') + 
  scale_y_origin()
g500 <- ggplot(flowstats_norm) + 
  geom_histogram(
    aes(x = PK0_2AEP), 
    color = 'black', fill = 'grey90', bins = sqrt(nrow(flowstats_norm))) + 
  scale_x_continuous('500-Year / 2-Year Flow Ratio') + 
  scale_y_origin()

ggarrange(g005, g010, g050, g100, g200, g500, nrow = 2, ncol = 3)

```

```{r include = FALSE}
## normalize w.r.t. 10-year peak
flowstats_norm <- flowstats %>% 
  column_to_rownames('site_no') %>% 
  apply(1, function(x) x/x[3]) %>% 
  t %>% as.data.frame %>% 
  rownames_to_column('site_no') 

ggplot(flowstats_norm) + 
  geom_histogram(
    aes(x = PK50AEP), 
    color = 'black', fill = 'grey90', bins = sqrt(nrow(flowstats_norm))) + 
  scale_x_continuous('2-Year / 10-Year Return Period Ratio') + 
  scale_y_origin()
ggplot(flowstats_norm) + 
  geom_histogram(
    aes(x = PK20AEP), 
    color = 'black', fill = 'grey90', bins = sqrt(nrow(flowstats_norm))) + 
  scale_x_continuous('5-Year / 10-Year Return Period Ratio') + 
  scale_y_origin()
ggplot(flowstats_norm) + 
  geom_histogram(
    aes(x = PK4AEP), 
    color = 'black', fill = 'grey90', bins = sqrt(nrow(flowstats_norm))) + 
  scale_x_continuous('25-Year / 10-Year Return Period Ratio') + 
  scale_y_origin()
ggplot(flowstats_norm) + 
  geom_histogram(
    aes(x = PK2AEP), 
    color = 'black', fill = 'grey90', bins = sqrt(nrow(flowstats_norm))) + 
  scale_x_continuous('50-Year / 10-Year Return Period Ratio') + 
  scale_y_origin()
ggplot(flowstats_norm) + 
  geom_histogram(
    aes(x = PK1AEP), 
    color = 'black', fill = 'grey90', bins = sqrt(nrow(flowstats_norm))) + 
  scale_x_continuous('100-Year / 10-Year Return Period Ratio') + 
  scale_y_origin()
ggplot(flowstats_norm) + 
  geom_histogram(
    aes(x = PK0_5AEP), 
    color = 'black', fill = 'grey90', bins = sqrt(nrow(flowstats_norm))) + 
  scale_x_continuous('200-Year / 10-Year Return Period Ratio') + 
  scale_y_origin()
ggplot(flowstats_norm) + 
  geom_histogram(
    aes(x = PK0_2AEP), 
    color = 'black', fill = 'grey90', bins = sqrt(nrow(flowstats_norm))) + 
  scale_x_continuous('500-Year / 10-Year Return Period Ratio') + 
  scale_y_origin()
...
```{r}
flowstats_norm %>% 
  pivot_longer(cols = -site_no, names_to = 'StatLabel', values_to = 'flow') %>% 
  left_join(labeldf, by = 'StatLabel') %>% 
  ggplot() + 
  geom_vline(xintercept = c(5,10,25,50,100,200,500), color = 'grey90') + 
  geom_line(aes(x = RP, y = flow, group = site_no), alpha = 0.25) + 
  scale_x_origin('Return Period (years)') + 
  scale_y_origin('Streamflow Peak / 2-Year Peak')
  
huc4 <- st_read('_data/WBD/WBDHU4.shp', quiet = TRUE)
flowstats_norm %>% 
  right_join(gages, ., by = 'site_no') %>% 
  ggplot() + 
  geom_sf(data = huc4, fill = NA) + 
  geom_sf(aes(color = PK0_2AEP)) + 
  scale_color_scico('100-Year / 2-Year', palette = 'bamako', direction = -1) + 
  theme_void() + 
  theme(text = element_text(family = 'Segoe UI'), legend.position = c(0.9,0.7))

```

## normalize streamflow by 2-year peak flows
Czajkowski, J., Villarini, G., Montgomery, M., Michel-Kerjan, E. O., & Goska, R. (2017). Assessing Current and Future Freshwater Flood Risk from North Atlantic Tropical Cyclones via Insurance Claims. In Nature (Vol. 7). https://doi.org/10.1038/srep41609

```{r}
flows <- flows %>% 
  left_join(flowstats %>% transmute(site_no, rp2 = PK50AEP), by = 'site_no') %>% 
  mutate(norm_rp2 = Flow/rp2)

```

```{r}
ggplot(flows %>% slice(sample(1:nrow(.), 1e6))) + 
  geom_point(aes(x = runoff, y = norm_rp2), alpha = 0.25) + 
  scale_x_origin('Runoff [mm]') + 
  scale_y_origin('Streamflow / 2-Year Peak []')
...
## calculate area for drainage geometries
poly1 <- drainage %>% 
  left_join(flowstats %>% select(site_no), by = 'site_no') %>% 
  st_make_valid %>% 
  mutate(area.1 = toNumber(st_area(.)))

## calculate area for HUC8 watersheds (same as above)
huc8 <- st_read('_data/WBD/WBDHU8.shp', quiet = TRUE)
poly2 <- huc8 %>%
  select(huc8) %>%
  mutate(area.2 = toNumber(st_area(.))) %>%
  ...
  foreach(
    i = 1:nrow(huc8)) %do% {
      ...
      if (length(gagelist) == 0) {
        ...
        norm_rp2 <- rep(NA, length(datelist))
        ...
        flowmatrix <- flows %>% 
          ...
          arrange(dateTime) %>% 
          pivot_wider(
            id_cols = 'dateTime', names_from = 'site_no', values_from = 'norm_rp2') %>% 
          select(-dateTime) %>% 
          as.matrix
        weightlist <- lookup %>% filter(huc == huc8$huc8[i]) %>% pull(prop.total)
        weightmatrix <- 
        ...
        ## assign weighted average to HUC8
        norm_rp2 <- (flowmatrix * weightmatrix) %>% apply(1, Sum)
      }
      data.frame(date = datelist, huc8 = huc8$huc8[i], norm_rp2)
    } %>% 
  reduce(rbind)
...
```{r}
huc_norm_rp2 <- huc8_norm_rp2 %>% 
  mutate(huc4 = str_sub(huc8, 1, 4)) %>% 
  group_by(date, huc4) %>% 
  mutate(norm_rp2_avg = Mean(norm_rp2)) %>% 
  mutate(norm_rp2 = ifelse(is.na(norm_rp2), norm_rp2_avg, norm_rp2)) %>% 
  ungroup
...
```{r}
## use conversion weights to list normalized streamflow by grid cell
norm_rp2 <- 
  foreach (i = 1:ncell(grid_ca)) %do% {
    huclist <- grid_convert %>% filter(cell==i) %>% pull(huc8)
    if (length(huclist) == 0) {
      ...
      huc_norm_rp2 %>% filter(huc8 == huclist) %>% select(date, norm_rp2)
    } else {
      hucmatrix <- suppressMessages(
        map_dfc(
          .x = huclist,
          .f = ~huc_norm_rp2 %>% filter(huc8 == .x) %>% pull(norm_rp2)
        ) %>% as.matrix)
      weightmatrix <- 
        matrix(
          data = grid_convert %>% 
      ...
      data.frame(
        date = datelist,
        cell = i,
        norm_rp2 = (hucmatrix * weightmatrix) %>% apply(1, Sum))
    }
  }
...
```{r}
## save out
save(norm_rp2, file = '_data/streamflow/norm_rp2_1002.Rdata')
