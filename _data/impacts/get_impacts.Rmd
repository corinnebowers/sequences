...
this file downloads & processes the impacts data included in df_merra
...
ncei <-
  foreach (yr = 1980:2020, .combine = 'rbind') %do% {
    list.files('_data/NCEI', full.names = TRUE) %>%
      .[grep(paste0('d',yr), .)] %>%
...
## download NWS climate zones
# https://www.weather.gov/gis/PublicZones
zones <- st_read('C:/Users/cbowers/Downloads/z_08se21/z_08se21.shp') %>%
  filter(STATE == 'CA') %>%
  st_transform(st_crs(california))

## subset zones
ncei_z <- ncei %>% filter(CZ_TYPE == 'Z')
# match(ncei_z$CZ_FIPS, toNumber(zones$ZONE)) %>% sum.na
# pretty high mismatch rate, but can't find older zone shapefiles --> use for now
ncei_z <- ncei_z %>%
  left_join(
    zones %>% transmute(ZONE = toNumber(ZONE), NAME), .,
    by = c('ZONE' = 'CZ_FIPS'))
...
** note: need to account for older zones **

### checkpoint

```{r}
# ## save out
# save(ncei, ncei_grid, file = '_data/impacts/files/ncei_0922.Rdata')

## load from file
load('_data/impacts/files/ncei_0922.Rdata')
...
# save(policies_save, file = '_data/impacts/files/policies_save.Rdata')

load('_data/impacts/files/policies_save.Rdata')
...
```{r}
## save out
save(claims, policies, file = '_data/impacts/files/NFIP_0922.Rdata')

# ## load from file
# load('_data/impacts/files/NFIP_0922.Rdata')
