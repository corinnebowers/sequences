...
catalog <- catalog.full %>%
  ...
  mutate(
    ...
    ncei_nonzero = ncei_damage > 0,  # %%%%%%%%%%%%%%%%% should this be different?
    nfip_nonzero = claims_value > 0) # %%%%%%%%%%%%%%%%% should this be different?
...
# ggplot(prox %>% filter(!is.na(or)) %>% mutate(p = add_index(nrow(.)))) + 
#   geom_step(aes(x = sort(or), y = p)) + 
#   geom_parity() + coord_fixed() + 
#   scale_x_origin() + scale_y_origin() + 
#   theme(panel.grid.major = element_line(size = 0.25))
# ggplot(prox %>% filter(!is.na(and)) %>% mutate(p = add_index(nrow(.)))) + 
#   geom_step(aes(x = sort(and), y = p)) + 
#   geom_parity() + coord_fixed() + 
#   scale_x_origin() + scale_y_origin() + 
#   theme(panel.grid.major = element_line(size = 0.25))
...
```{r}
## what about timelines other than 5 days? 
...
g3 <- g.right + 
  ...
  #       label = c('AR1', 'AR2', NA, NA, NA)),
  #   aes(x = 5.75, y = placement, label = label, color = color),
  #   family = 'Segoe UI', size = 8/.pt, fontface = 'bold', show.legend = FALSE) +
  scale_y_continuous(labels = percent, expand = expansion(mult = 0)) + 
...
g4 <- g.right + 
  ...
  #   aes(x = 5.75, y = placement+0.005, label = paste0('AR',color), color = color),
  #   family = 'Segoe UI', size = 8/.pt, fontface = 'bold', show.legend = FALSE) +
  # geom_text(
  #   data = data.frame(ratio = ratios$med, color = factor(c(1:5), levels = 5:1)) %>% 
  #     mutate(placement = ratio - diff(c(0,ratio))/2),
  #   aes(x = 5.75, y = placement[3]+0.005, label = paste0('AR',color)[3]), color = '#bfd8b8',
...
ggsave('_figures-impacts/fig2_prevcat.png', width = 3.55, height = 4.25, units = 'in', dpi = 500)
...
summary(f2, robust = TRUE) %>% 
  ...
  coord_flip()
ggsave('_figures-impacts/fig3_likelihood.png', width = 3.55, height = 2.5, dpi = 500)
...
ggsave('_figures-impacts/fig4_magnitude.png', width = 7.25, height = 3.5, dpi = 500)
...
```{r}
## which ARs do not have a previous AR within 30 days? 

catalog %>% 
  ggplot() + 
  geom_histogram(aes(x = prev.inter), bins = sqrt(nrow(catalog)), color = NA, fill = 'grey25') + 
  scale_x_origin() + scale_y_origin()

catalog %>% filter(prev.inter > 1e3)
catalog %>% filter(id == 173)
grid_ca %>% 
  raster.df %>% 
  ggplot() +
  geom_raster(aes(x=x, y=y, fill = value==173)) + 
  geom_sf(data = california, fill = NA)

catalog %>% 
  ggplot() + 
  geom_bar(aes(
    x = factor(month(ar.start), levels = c(10:12,1:9)), 
    fill = cut(prev.inter, c(0,30,180,Inf)))) + 
  scale_x_discrete(
    labels = c('Oct','Nov','Dec','Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep')) + 
  scale_y_origin('Number of Observations', labels = comma) + 
  scale_fill_manual(
    'Time Since\nPrevious Event',
    values = c('grey90','grey60','grey30'), 
    labels = c('<30 Days', '30-180 Days', '>180 Days')) + 
  theme(
    text = element_text(family = 'Segoe UI', size = 9),
    axis.title.x = element_blank(), 
    panel.grid.major.y = element_line(size = 0.25),
    legend.position = c(0.8,0.8))
ggsave('_figures-impacts/reviewer_figa.png', width = 3.55, height = 3)

sum(catalog$prev.inter > 30) / nrow(catalog)
sum(catalog$prev.inter > 180) / nrow(catalog)
...
ggsave('_figures-impacts/fig5_previous.png', width = 7.25, height = 5.75, dpi = 500)
...
```{r}
## combine previous category with primary category
boot <- 100
...
coef.ncei <- 
  foreach (b = 1:boot) %do% {
    index.sample <- c(index.ncei.nonzero, sample(
      index.ncei.zero, size = 2*length(index.ncei.nonzero), replace = FALSE))
    felm(
      log10(ncei_damage+1) ~ factor(ar.cat) + factor(ar.cat):factor(prev.cat) | factor(id) + factor(wy),
      data = catalog %>% 
        mutate(
          prev.cat = ifelse(prev.inter <= 5, prev.cat, 0),
          prev.cat = ifelse(prev.cat==5, 4, prev.cat),
          ar.cat = ifelse(ar.cat==5, 4, ar.cat)) %>% 
        slice(index.sample), 
      exactDOF = TRUE) %>% 
      summary %>% coef
  } %>% 
...
coef.nfip <- 
  foreach (b = 1:boot) %do% {
    index.sample <- c(index.nfip.nonzero, sample(
      index.nfip.zero, size = 2*length(index.nfip.nonzero), replace = FALSE))
    felm(
      log10(claims_value+1) ~ factor(ar.cat) + factor(ar.cat):factor(prev.cat) | factor(id) + factor(wy),
      data = catalog %>% 
        mutate(
          prev.cat = ifelse(prev.inter <= 5, prev.cat, 0),
          prev.cat = ifelse(prev.cat==5, 4, prev.cat),
...
```{r}
coef.new %>% 
  rbind(expand.grid(ar.cat = 1:4, prev.cat = 0, source = c('NCEI','NFIP'), est = 1, lower = 1, upper = 1)) %>% 
  mutate(ar.cat = factor(ar.cat)) %>% 
  ggplot() +
  geom_hline(yintercept = 1, color = 'grey50') + 
  geom_ribbon(aes(x = prev.cat, ymin = lower, ymax = upper, group = ar.cat, fill = ar.cat), alpha = 0.25) +
  geom_point(aes(x = prev.cat, y = est, group = ar.cat, color = ar.cat, shape = ar.cat), size = 1.5) +
  geom_line(aes(x = prev.cat, y = est, group = ar.cat, color = ar.cat), size = 0.75) +
  scale_x_continuous(breaks = 0:4, labels = c('No Previous\nEvent', paste0('AR',c(1,2,3,'4-5')))) + 
  scale_y_log10(labels = comma_format(suffix = 'x')) + logticks('l') + 
  scale_color_manual(
    'Primary Event Rank', labels = paste0('AR',c(1,2,3,'4-5')), 
    values = roma.colors[c(5,4,2,1)]) +
  scale_fill_manual(
    'Primary Event Rank', labels = paste0('AR',c(1,2,3,'4-5')), 
    values = roma.colors[c(5,4,2,1)]) +
  scale_shape_manual(
  ...
  theme(
    ...
    legend.key.width = unit(0.25, 'in'),
    legend.position = 'bottom')
ggsave('_figures-impacts/reviewer_figc.png', width = 6, height = 3.5)
...
ggsave('_figures-impacts/fig6_position.png', width = 7.25, height = 4, dpi = 500)
