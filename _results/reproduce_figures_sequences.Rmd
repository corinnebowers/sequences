...
p.precip <- 
  foreach (i = 1:ncell(grid_ca), .combine = 'rbind') %do% {
    ...
    if (i %in% index_ca) {
      dx <- 25
      df_3hr[[i]] %>% 
        group_by(ivt.bucket = round(ivt/dx)*dx) %>% 
        count(wet = precip > 1e-2) %>% 
        mutate(pct = n/sum(n)) %>% 
        ungroup %>% filter(wet) %>% 
        transmute(ivt.bucket, pct, id = i) 
    }
  }

left_join(raster.df(grid_ca), raster.df(grid_huc4), by = c('x','y')) %>% 
  rename(id = value.x, huc4 = value.y) %>% 
  filter(!is.na(huc4)) %>% left_join(p.precip) %>% 
  ggplot() + 
  geom_hline(yintercept = 0.5, color = 'grey80') +
  geom_vline(xintercept = 250, color = 'grey80') + 
  geom_line(
    aes(x = ivt.bucket, y = pct, group = factor(id)), size = 0.25) + 
  facet_wrap(~paste('HUC',huc4), nrow = 2) + 
  scale_x_origin('IVT (kg/m/s)', breaks = 250*(0:10)) +
  scale_y_origin('Probability of Precipitation', labels = percent) + 
  theme(
    text = element_text(family = 'Segoe UI', size = 10),
    axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
    panel.grid.major = element_line(size = 0.25),
    strip.background = element_rect(fill = 'grey95', color = NA),
    strip.text = element_text(family = 'Segoe UI', margin = margin(2,2,2,2)))
ggsave('C:/Users/cbowers/Desktop/ppt_prob.png', width = 6, height = 3, dpi = 600)
